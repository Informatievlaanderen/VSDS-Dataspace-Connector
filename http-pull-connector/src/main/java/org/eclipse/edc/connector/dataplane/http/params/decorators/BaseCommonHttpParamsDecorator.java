...
import static org.eclipse.edc.connector.dataplane.http.spi.HttpDataAddress.ADDITIONAL_HEADER;
...
public class BaseCommonHttpParamsDecorator implements HttpParamsDecorator {
    ...
    public HttpRequestParams.Builder decorate(DataFlowRequest request, HttpDataAddress address, HttpRequestParams.Builder params) {
        ...
        request.getProperties().forEach((key, value) -> {
            if (key.startsWith(ADDITIONAL_HEADER)) {
                params.header(key.replace(ADDITIONAL_HEADER, ""), value);
            }
        });
        return params
                .baseUrl(baseUrl)
